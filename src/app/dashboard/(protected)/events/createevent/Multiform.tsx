  ...
  const handleStep3 = async() => {
   ...
   return (
    <div className="max-w-xl mx-auto p-6 border rounded">
      {step === 1 && <Step1 data={formData} updateField={updateField} />}
      {step === 2 && <Step2 data={formData} updateField={updateField} />}
      {step === 3 && <Step3 data={formData} updateField={updateField} />}

      <div className="mt-4 flex justify-between">
        {step > 1 && <button onClick={() => setStep(step - 1)}>Back</button>}

        {step === 1 && <button onClick={handleStep1} disabled={loading}>{loading ? "Saving..." : "Next"}</button>}
        {step === 2 && <button onClick={handleStep2} disabled={loading}>{loading ? "Uploading..." : "Next"}</button>}
        {step === 3 && <button onClick={handleStep3} disabled={loading}>{loading ? "Saving..." : "Finish"}</button>}
      </div>
    </div>
  );
}
